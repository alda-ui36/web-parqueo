...
import { metodoPagoSchema, type MetodoPagoSchema } from "@/schemas/metodo-pago.schema";
...
export const useMetodoPago = () => {
    ...
    const [editingMetodo, setEditingMetodo] = useState<MetodoPagoResponse | null>(null);
    ...
    const loadMetodos = async (page = 0, size = 5, query?: string, estado?: string) => {
        try {
            ...
            const errorMessage = error.response?.data?.mensaje || error.message || 'Error al cargar métodos de pago';
    ...
    const handleSubmit = async (values: MetodoPagoSchema) => {
        ...
        try {
            ...
            const errorMessage = error.response?.data?.mensaje || error.message || 'Error al guardar método de pago';
            toast.error(errorMessage);
        } finally {
            setSubmitting(false);
        }
