...
import { espacioSchema, type EspacioSchema } from "@/schemas/espacio.schema";
...
export const useEspacio = () => {
    ...
    const [editingEspacio, setEditingEspacio] = useState<EspacioResponse | null>(null);
    ...
    const loadEspacios = async (page = 0, size = 5, query?: string, estado?: string) => {
        try {
            ...
            const errorMessage = error.response?.data?.mensaje || error.message || 'Error al cargar espacios';
    ...
    const handleSubmit = async (values: EspacioSchema) => {
        ...
        try {
            ...
            const errorMessage = error.response?.data?.mensaje || error.message || 'Error al guardar espacio';
            toast.error(errorMessage);
        } finally {
            setSubmitting(false);
        }
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                const tiposRes = await tipoVehiculoService.findAll({ page: 0, size: 1000 });
                setTiposVehiculo(tiposRes.data.content.map((t: any) => ({ id: t.id, nombre: t.nombre })));
            } catch (e) {
    ...
    const getEspaciosDisponibles = (zonaId: number) => {
        ...
        const activos = espacios.filter(e => e.zonaId === zonaId && e.estado).length;
