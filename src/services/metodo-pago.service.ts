...
export const metodoPagoService = {
  async create(request: MetodoPagoRequest): Promise<ApiResponse<MetodoPagoResponse>> {
    const response = await mainApi.post("/metodos-pago", request);
    return response.data;
  },

  async update(id: number, request: MetodoPagoRequest): Promise<ApiResponse<MetodoPagoResponse>> {
  ...
  async findAll(filters: MetodoPagoFilters = {}): Promise<ApiResponse<MetodoPagoPageResponse>> {
