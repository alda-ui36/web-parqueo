      ...
      render: (value: boolean) => (
        <Badge
          ...
          className={`w-20 justify-center text-white ${
            value ? "bg-red-100 text-red-500" : "bg-green-100 text-green-500"
          }`}
        >
          {value ? "Sí" : "No"}
        </Badge>
  ...
  const actions = (espacio: any) => (
    <div className="flex gap-2">
      ...
      <Button
        variant="outline"
        size="sm"
        onClick={() => toggleStatus(espacio.id)}
      >
        <Power
          className={`h-4 w-4 ${
            espacio.estado ? "text-red-500" : "text-green-500"
          }`}
        />
      </Button>
    </div>
    ...
    <div className="container mx-auto py-6">
      ...
      <DataTable
        ...
        pagination={{
          ...
          onPageChange: (p) =>
            loadEspacios(p, pagination.pageSize, undefined, estadoFilter),
